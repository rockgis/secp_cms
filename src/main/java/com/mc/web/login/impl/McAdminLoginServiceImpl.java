...
public class McAdminLoginServiceImpl extends EgovAbstractServiceImpl implements McAdminLoginService{
	...
	public Map check_id(Map<String, String> params) throws Exception {
		...
		if(member == null){//아이디 없음
		    ...
		    if ("Y".equals(member.get("made_yn"))) { // 관리자가 생성 후 비밀번호를 변경하지 않은 경우
		        map.put("rst", "M");
		        return map;
		    } 
			MCMap basicMap = basicDAO.basic_view("1");
			if(basicMap!=null){
				if("Y".equals(basicMap.getStrNull("adm_pw_change_yn"))){//비밀번호 변경 사용여부 확인
					if(!"".equals(member.getStrNullVal("last_pw_dt", ""))){
						...
						int pw_change_cycle = Integer.parseInt(DateUtil.getCurrentDateBDay(basicMap.getIntNullVal("adm_pw_change_cycle", 90)));//비밀번호 변경주기만큼 뺀 날짜
				...
				if(!"pass".equals((String)session.getAttribute("dormancy")) && "Y".equals(basicMap.getStrNull("adm_dormancy_yn"))){//장기 미접속자 사용여부 확인
	        ...
	        if(sessionInformationSupport.userExists(params.get("member_id"), "admin")){//이미 로그인된 사용자가 존재함.
	            map.put("rst", "Y");
	        }else{
	            map.put("rst", "N");
	        }
