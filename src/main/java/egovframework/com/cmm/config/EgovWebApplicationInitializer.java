 ...
 *   2020.05.08  이창기           mccms web.xml 옮겨옴
 * </pre>
 */
...
public class EgovWebApplicationInitializer implements WebApplicationInitializer {
	...
	public void onStartup(ServletContext servletContext) throws ServletException {
		...
		//rootContext.setConfigLocations(new String[] { "classpath*:spring/context-*.xml","classpath*:egovframework/spring/com/**/context-*.xml" });
//		rootContext.setConfigLocations(new String[] { "classpath*:egovframework/spring/com/**/context-*.xml" });
		...
		xmlWebApplicationContext.setConfigLocations(new String[] { "classpath*:spring/appServlet/servlet-context.xml" });
		//xmlWebApplicationContext.setConfigLocations(new String[] { "classpath*:spring/appServlet/servlet-context.xml","/WEB-INF/config/egovframework/springmvc/egov-com-*.xml" });
//		xmlWebApplicationContext.setConfigLocation("/WEB-INF/config/egovframework/springmvc/egov-com-*.xml");
		...
		ServletRegistration.Dynamic initServlet = servletContext.addServlet("InitServlet", new com.mc.web.common.InitServlet());
...
/*
		...
		*/
		...
		FilterRegistration.Dynamic sitemesh = servletContext.addFilter("sitemesh", new com.mc.web.filter.McSiteMeshFilter());
		sitemesh.addMappingForUrlPatterns(EnumSet.of(DispatcherType.REQUEST), true, "*.do");
		sitemesh.addMappingForUrlPatterns(EnumSet.of(DispatcherType.REQUEST), true, "*.jsp");
		
		//-------------------------------------------------------------
		// Tomcat의 경우 allowCasualMultipartParsing="true" 추가
		// <Context docBase="" path="/" reloadable="true" allowCasualMultipartParsing="true">
		MultipartFilter springMultipartFilter = new MultipartFilter();
		springMultipartFilter.setMultipartResolverBeanName("filterMultipartResolver");
		FilterRegistration.Dynamic multipartFilter = servletContext.addFilter("springMultipartFilter", springMultipartFilter);
		multipartFilter.addMappingForUrlPatterns(null, false, "*.do");
		...
		springSecurityFilterChain.addMappingForUrlPatterns(null, false, "*.do");
		...
		//FilterRegistration.Dynamic ClickjackFilterSameOrigin = servletContext.addFilter("ClickjackFilterSameOrigin", new com.mc.web.filter.ClickjackFilter());
		//ClickjackFilterSameOrigin.addMappingForUrlPatterns(null, false, "/*");
		//ClickjackFilterSameOrigin.setInitParameter("mode", "SAMEORIGIN");
		
		servletContext.addListener(new org.springframework.security.web.session.HttpSessionEventPublisher());
		
		ServletRegistration.Dynamic ajaxServlet = servletContext.addServlet("ajaxServlet", new net.sourceforge.ajaxtags.servlets.SourceLoader());
		ajaxServlet.setInitParameter("prefix", "/ajaxtags");
		ajaxServlet.addMapping("/ajaxtags/js/*");
		ajaxServlet.addMapping("/ajaxtags/img/*");
		ajaxServlet.addMapping("/ajaxtags/css/*");
