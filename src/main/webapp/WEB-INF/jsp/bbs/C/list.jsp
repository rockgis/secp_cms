...
<table class="board_list1">
	...
	<thead>
		<tr>
			...
			<c:if test="${pagination.cat_yn == 'Y' }"><th scope="col">질문유형</th></c:if>
			<th scope="col">제목</th>
			<th scope="col" class="none">작성자</th>
			<th scope="col">상태</th>
	...
	<c:forEach items="${notice_list }" var="item" varStatus = "status">
		<tr>
			...
			${suf:clearXSS(item.title, '')}<c:if test="${item.public_yn == 'N' }"><img src="/images/board/style01_mp.gif" alt="비공개" style="margin-left: 5px;"></c:if><c:if test="${item.comment_cnt > 0}"> (${item.comment_cnt })</c:if>
			</a>
			</td>
			<td class="none">${(item.member_id == '' || item.member_id == null) ? item.reg_nm : item.member_id}</td>
			<td>${item.state_nm }</td>		
