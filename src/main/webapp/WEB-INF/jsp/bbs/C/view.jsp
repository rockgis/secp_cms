...
<table class="board_view">
	...
	<tbody>
		...
		<tr>
		  <td colspan="4" class="contents">
		  	${fn:replace(fn:replace(view.reply_conts, '<script>', '&lt;script&gt;'), '</script>', '&lt;/script&gt;')}
		  </td>
		</tr>
		</c:if>
	</tbody>
...
<div class="btn_area">
	...
	<c:if test="${(sessionScope.member != null && sessionScope.member.member_id == view.reg_id) || view.password != null}">
		...
		<a class="v3" href="list.do?cpage=${suf:requestReplace(params.cpage , '')}&amp;rows=${suf:requestReplace(params.rows , '')}&amp;condition=${suf:requestReplace(params.condition , '')}&amp;keyword=${suf:requestReplace(params.keyword , '')}&amp;cat=${suf:requestReplace(params.cat, '')}">목록</a>
	</div>
</div>
