        ...
        <div class="sub-content">
            <div class="board-list-wrap">
                ...
                <table class="tbl-list2 hoverbg">
                    ...
                    <caption>1:1 문의 목록 : 번호, 제목, 작성자, 작성일, 첨부파일로 구성된 표</caption>
                    <thead>
                        <tr>
                            <th scope="col">번호</th>
                            <th scope="col">제목</th>
                    ...
                    <tbody>
                    	<c:choose>
	                    	...
	                    	<c:otherwise> 
		                    	<c:forEach var="InquiryList" items="${boardList}">
		                        <tr>
		                            <td data-label="번호">${InquiryList.RN}</td>
		                            <td data-label="제목" class="t-left"><a href="/board/boardDetail.do?as=${InquiryList.ARTICLE_SEQ}&type=3">${InquiryList.TITLE}</a><img src="../images/secp/ico_new.svg" alt="새글" class="ico"></td>
		                            <td data-label="작성자">${InquiryList.REG_NM}</td>
		                            <td data-label="작성일">${InquiryList.REG_DT2}</td>
		                            <td data-label="답변일">${InquiryList.REPLY_DT}</td>
                ...
                <div class="t-right">
                    <a href="/board/boardInquiryRegisterForm.do" class="btn btn-blue">글쓰기</a>
                </div>

				<div class="page">
					<c:if test="${fn:length(boardList) > 0}">
					<ul class="btn-group pagination">
						<li><a href="/board/boardIndex.do?page=1&type=3" class="img"><img src="../images/secp/ico_page_first.svg" alt="처음으로"></a></li>
						<c:choose>
					    	<c:when test="${cri.page > pageMaker.startPage}">
					    		<li><a href="/board/boardIndex.do?page=${cri.page-1}&type=3" class="img mr10"><img src="../images/secp/ico_page_prev.svg" alt="이전으로"></a></li>
					    	</c:when>
					    	<c:when test="${cri.page <= pageMaker.startPage}">
					    ...
					    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="pageNum">
						    <li>
						        <a <c:if test="${cri.page eq pageNum}">class="active"</c:if> href="<c:url value='/board/boardIndex.do?page=${pageNum}&type=3'/>">${pageNum}</a>
						    </li>
					    </c:forEach>
					    
					    <c:choose>
					    	<c:when test="${cri.page < pageMaker.endPage}">
					    		<li><a href="/board/boardIndex.do?page=${cri.page+1}&type=3" class="img ml10"><img src="../images/secp/ico_page_next.svg" alt="다음으로"></a></li>
					    	</c:when>
					    	<c:when test="${cri.page >= pageMaker.endPage}">
					    		<li><a href="#" class="img ml10"><img src="../images/secp/ico_page_next.svg" alt="다음으로"></a></li>
					    	</c:when>
					    </c:choose>
					    <li><a href="<c:url value="/board/boardIndex.do?page=${pageMaker.endPage}&type=3"/>" class="img"><img src="../images/secp/ico_page_last.svg" alt="마지막으로"></a></li>
					</ul>
					</c:if>
				</div>
