...
app.register.controller("listCtrl", function($scope, $timeout, $window, $routeParams, $location, ajaxService) {
	...
	ajaxService.getSyncJSON('<c:url value="/super/bbs/C/list.do"/>', $scope.param, function(data){
	...
	if($scope.statelist.length==0){
		ajaxService.getSyncJSON('<c:url value="/super/bbs/C/stateList.do"/>', {board_seq : $scope.param.board_seq}, function(data){
			$scope.$emit("putStatelist", {list : data}); 
		});
	}
