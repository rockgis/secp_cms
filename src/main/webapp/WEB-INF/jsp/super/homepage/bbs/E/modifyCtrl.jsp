...
app.register.controller("modifyCtrl", function($scope, $timeout, $window, $routeParams, ajaxService, wordFilterService) {
	...
	ajaxService.getJSON("<c:url value="/super/bbs/E/view.do"/>", {article_seq : $routeParams.seq, board_seq : $scope.param.board_seq, del_yn : $scope.param.del_yn}, function(data){
	...
	$scope.save = function(){
		...
		promise.then(function(){
			ajaxService.getJSON("<c:url value="/super/bbs/E/modify.do"/>", {jData : angular.toJson($scope.form)}, function(data){
				if(data.rst == -1){
					alert(data.msg)
				}else{
					$scope.list();
