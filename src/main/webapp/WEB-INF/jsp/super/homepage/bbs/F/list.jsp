...
<div class="contents_wrap">
	<div class="ct_wrap">
		...
		<div class="contents" data-ng-init="board.go(param.cpage)">
	    	...
	    	<div>
				<div>
					<span class="bt_all">
						...
						<span data-ng-class="{active : param.del_yn == 'Y'}"><input type="button" value="삭제 게시물 관리" data-ng-click="param.del_yn = 'Y';param.gubun = '';board.go(1);"/></span>
				...
				<table class="type1">
					...
					<tr ng-if="param.cat_yn=='Y'">
						<th>카테고리</th>
						<td>
							<select class="normal w175" title="카테고리" data-ng-model="param.cat" data-ng-options="item.board_cat_seq as item.cat_nm for item in catlist" data-ng-if="param.cat_yn == 'Y'">
				...
				<div style="overflow:hidden; margin-top:35px;">
			        ...
			        <div class="right_box_cms">
			        	<select class="normal w100" title="건별보기 선택" data-ng-model="param.rows" data-ng-init="param.rows=10">
			...
			<table class="type1" id="boardList" >
				<colgroup>
					<col width="5%" />
					<%-- <col width="{{item.admin_list_col}}" data-ng-repeat="item in custom" data-ng-if="item.column_name != 'notice_yn' && item.column_name != 'password'"/> --%>
					<col width="{{item.admin_list_col}}" data-ng-repeat="item in custom" />
					<col width="6%" data-ng-if="param.file_yn == 'Y'"/>
					<col width="6%" />
	        	</colgroup>
				<tr>
					<th class="center"><input type="checkbox" data-ng-model="board.chk_all" value="on" data-ng-change="chk_all_btn()"/></th>
					<!-- <th class="center" data-ng-repeat="item in custom" data-ng-if="item.column_name != 'notice_yn' && item.column_name != 'password'">{{item.element}}</th> -->
					<th class="center" data-ng-repeat="item in custom" >{{item.element}}</th>
					<th class="center" data-ng-if="param.file_yn == 'Y'">첨부파일</th>
					<th class="center">신고수</th>
				</tr>
				<tr data-ng-if="board.list.length==0"><td colspan="{{returnColspan(custom)+1}}" class="center">결과가 없습니다.</td></tr>
				<tr data-ng-repeat="item in board.list">
					<td class="center">
						<input type="checkbox" value="{{item.article_seq}}"/>
					</td>
					<!-- <td class="{{cols.column_name == 'title' ? 'left' : 'center'}}" data-ng-repeat="cols in custom" data-ng-if="cols.column_name != 'notice_yn' && cols.column_name != 'password'">
						<a data-my-href="/modify/F/{{item.article_seq}}" ng-bind-html="itemBind((board.totalCount|num) + 1 - (item.rn|num),cols,item)"></a>
					</td> -->
					<td class="{{cols.column_name == 'title' ? 'left member_nm' : 'center'}}" data-ng-repeat="cols in custom" >
						<a data-my-href="/modify/F/{{item.article_seq}}" ng-bind-html="itemBind((board.totalCount|num) + 1 - (item.rn|num),cols,item)"></a>
					</td>
