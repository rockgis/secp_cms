...
<script type="text/javascript" src="<c:url value="/js/admin/pdf_create.js"/>"></script>
<script type="text/javascript" src="<c:url value="/js/admin/biz001/introB.js"/>"></script>
	...
	function createAUIGrid(columnLayout) {
		...
		var gridPros = {
			...
			rowCheckableFunction: function (rowIndex, isChecked, item) {
				if (item.STTUS != "임시저장") { // 상태가 접수완료 경우만 체크 하도록 함.
					return false;
				}
				return true;
			},
			...
			rowCheckDisabledFunction: function (rowIndex, isChecked, item) {
				if (item.STTUS != "임시저장") { // 제상태가 접수완료가 아닌 경우만 체크 하도록 경우 체크박스 disabeld 처리함
				...
				<div class="f-left">
					<form name=f1 style=margin:0; onsubmit="return false;" >
						<select name="codeList" id="codeList" onchange="OnChange();">
							<option selected value="A">1.전체 평가현황</option>
							<option value="B">2.정성 평가 결과 업로드</option>
						</select>
						<button type="button" id="allpdf" class="btn btn-mint" onclick="createPdfServer('/super/biz001/gridToPdf.do','biz001');">전체 평가자료 PDF 생성</button>
					</form>
