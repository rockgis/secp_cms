		...
		<div class="contents">
			<form name="searchFrm" method="post" data-ng-submit="board.go(1)">
				<table class="type1" style="margin-top:0;">
					...
					<tr>
						<th>구분</th>
						<td>
							<select data-ng-model="param.gubun" class="normal w100">
					...
					<tr>
						<th>검색어</th>
						<td>
							<select data-ng-model="param.condition" id="condition" class="normal w100">
								<option value="COMPANY">소속/회사</option>
							</select>
			...
			<table class="type1" id="boardList">
				...
				<tr>
					...
					<th class="center">소속/회사</th>
					<th class="center">빌드ID</th>
					<th class="center">발급일</th>
				...
				<tr data-ng-repeat="item in board.list">
					<td class="center"><input type="checkbox" value="{{item.seq}}"/></td>
					<td class="center">{{(board.totalCount|num) + 1 - (item.rn|num)}}</td>
					<td class="center">{{item.gubun=='0'?'개발':'운영'}}</td>
					<td class="center">{{item.company}}</td>
					<td class="center">{{item.build_id}}</td>
					<td class="left">{{item.issued_dt|myDate:'yyyy/MM/dd'}}</td>
					<td class="center">{{item.name}}</td>
					<td class="center">
						<span class="bt_alls">
							<span><input type="button" value="수정" class="btalls" data-my-href="/modify/{{item.seq}}"></span>
							<span><input type="button" value="키 발급" class="btalls" ng-click="keyIssue(item.seq)"></span>
						</span>
					</td>
				</tr>
			</table>
