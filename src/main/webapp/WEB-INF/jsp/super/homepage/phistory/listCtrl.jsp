...
app.register.controller("listCtrl", function($scope, $timeout, $window, $routeParams, $location, ajaxService, dialogService) {
	...
	angular.extend($scope.param, {
		rows : 10,
		cpage : p.cpage||1,
		condition : p.condition||'A.REG_DT',
		keyword : p.keyword||''
	});
	ajaxService.getSyncJSON('<c:url value="/super/phistory/list.do"/>', $scope.param, function(data){
	...
	$scope.del = function(item){
		...
		ajaxService.getJSON("<c:url value="/super/phistory/del.do"/>", item, function(data){
			if(data.rst == '1'){
				alert("정상 처리되었습니다.");
				$scope.board.go(1);
			}else{
