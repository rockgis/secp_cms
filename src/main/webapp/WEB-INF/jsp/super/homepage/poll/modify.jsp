	    ...
	    <form id="wFrm" name="frm" method="post" novalidate="novalidate" enctype="multipart/form-data">
	      	<table class="type1" style="margin-top:0;">
		        ...
		        <tr>
		          	...
		          	<td>
		          		<input type="text" class="normal w175" style="position:relative; z-index:100; margin-right:3px;" data-ng-model="form.start_dt" datetimepicker required/>
		          	 	<span>~</span>
		          	 	<input type="text" class="normal w175" style="position:relative; z-index:100; margin-right:3px;" data-ng-model="form.end_dt" datetimepicker required/></td>
		        </tr>
		        <tr>
				  	<th>중복설문여부</th>
				  	<td>
						<ol class="select" style="line-height:25px;">
							<li class="first"><label><input type="radio" data-ng-model="form.dup_yn" value="Y"/> 사용</label></li>
				...
				<tr>
					...
					<td>
						<ol class="select" style="line-height:25px;">
							<li class="first"><label><input type="radio" data-ng-model="form.lot_yn" value="Y" data-ng-init="form.use_yn = 'Y'"/> 사용</label></li>
				...
				<tr data-ng-repeat="item in form.question|filter:{del_yn:'N'}">
					<th>
						<div data-ng-if="item.subject_yn == 'N'">
							{{item.cnt}}번 설문
							<span style="margin-left:3px;"><input type="button" value="설문삭제" class="btalls" data-ng-click="removeQuestion(item, $index, 'N');" /></span><br/>
						...
						<div data-ng-if="item.subject_yn == 'Y'">
							소제목<span style="margin-left:3px;"><input type="button" value="소제목삭제" class="btalls" data-ng-click="removeQuestion(item, $index, 'Y');" /></span>
						</div>
					</th>
					<!-- <th>
						{{$index+1}}번 설문
						<span data-ng-if="$index > 0" style="margin-left:3px;"><input type="button" value="설문삭제" class="btalls" data-ng-click="removeQuestion(item,$index);" /></span>
					</th> -->
					<td>
						<ul id="questionWrap" class="question">
							...
							<li data-ng-repeat="sub_item in item.answers" data-ng-if="sub_item.del_yn == 'N' && item.subject_yn == 'N'">
		  	...
		  	<div class="btn_bottom">
	        	<div class="r_btn">
	          		...
	          		<input type="button" value="조사 결과 초기화" class="bt_big_bt2" data-ng-click="initResult()"/>
	        	</div>
	      	</div>
		</form>
	    </div>
