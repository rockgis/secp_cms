<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<div id="memberEditDiv" ng-controller="memberEditCtrl" title="회원 등록">
        ...
        <table class="type1">
			...
			<tr>
        		...
        		<td style="white-space:normal;" data-ng-if="model.mode=='ADD'"><input type="text" style="width:100%;" id="member_id" name="member_id" data-ng-model="form.member_id" class="normal" data-ng-minlength="4" autocomplete="off" required="required" idcheck/>
					<div style="margin-top:5px;" data-ng-show="memberFrm.member_id.$dirty && memberFrm.member_id.$error.idcheck"><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 0px 0;"></span>
			...
			<tr>
				<th>직위 <b class="essential">*</b></th>
	       		<td>
					<input type="text" class="normal" data-ng-model="form.positions" style="width:100%;" required="required" />
				</td>
			</tr>
        	<tr data-ng-if="model.mode=='ADD'">
        		...
        		<td style="white-space:normal;">
					...
					<div style="margin-top:5px; " ng-show="memberFrm.member_pw.$dirty && memberFrm.member_pw.$invalid">
        	...
        	<!-- 1,2번시 담당페이지, 페이지 권한 기능 숨김 -->
        	<tr ng-if="form.group_seq>'2'">
        		<th>담당페이지</th>
        		<td>
        			<button value="초기화" class="bt_small refresh" data-ng-click="staff_init()" onclick="return false;">초기화</button>
					<button value="변경" class="bt_small modify" style="margin-left:3px;" data-ng-click="staff(form)" onclick="return false;">변경</button>
        		</td>
        	</tr>
        	<tr ng-if="form.group_seq>'2'">
        		<th>페이지권한</th>
        		<td>
        			<button value="초기화" class="bt_small refresh" data-ng-click="permission_init()" onclick="return false;">초기화</button>
					<button value="변경" class="bt_small modify" style="margin-left:3px;" data-ng-click="permission(form)" onclick="return false;">변경</button>
        		</td>
        	</tr>
        	<!-- 1,2번시 담당페이지, 페이지 권한 기능 숨김 -->
        	...
        	<tr>
        		...
        		<td>
					<select title="전화번호 앞자리 선택" class="normal" style="width:60px" data-ng-model="form.tel1" data-ng-options="item.code as item.code for item in main.tel1" data-ng-init="form.tel1 != null && form.tel1 != '' ? form.tel1 : form.tel1=main.tel1[0].code" required="required">
            		</select> - 
            		<input type="text" class="normal w100" data-ng-model="form.tel2" maxlength="4" data-ng-pattern="/[0-9]{3,4}/" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" required="required" /> - 
            		<input type="text" class="normal w100" data-ng-model="form.tel3" maxlength="4" data-ng-pattern="/[0-9]{4}/" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" required="required" />
				</td>         	
			</tr>
        	<tr>
        		<th>휴대폰</th>
        		<td>
					<select title="휴대폰 앞자리 선택" class="normal" style="width:60px" data-ng-model="form.cell1" data-ng-options="item.code as item.code for item in main.cell1" data-ng-init="form.cell1 != null && form.cell1 != '' ? form.cell1 : form.cell1=main.cell1[0].code">
            		</select> - 
            		<input type="text" class="normal w100" data-ng-model="form.cell2" maxlength="4" data-ng-pattern="/[0-9]{3,4}/" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"/> - 
            		<input type="text" class="normal w100" data-ng-model="form.cell3" maxlength="4" data-ng-pattern="/[0-9]{4}/" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"/>
				</td>         	
			</tr>
        	<tr>
			...
			<tr>
				<th>담당업무</th>
	       		<td>
					<textarea style="width:100%; height:100px; resize:none; overflow-y:scroll; border:solid 1px #ddd; box-sizing:border-box;" data-ng-model="form.responsibilities"></textarea>
				</td>
			</tr>
		</table>
		...
		<div class="btn_bottom">
		  	<div class="r_btn">
		  		<input type="button" value="삭제" class="bt_big_bt3" data-ng-click="del(form)" data-ng-if="model.mode=='MOD'">
