	...
	<div ng-controller="ModifyPwCtrl" title="그룹선택">
        ...
        <table class="type1">
		    ...
		    <tr>
				<th>비밀번호</th>
        		<td>
					<input type="password" style="width:100%;" data-ng-model="form.member_pw" id="member_pw" name="member_pw" class="normal" maxlength="20" data-ng-minlength="6" data-ng-maxlength="20" data-ng-pattern="/(^.*(?=.{8,15})(?=.*[0-9])(?=.*[a-zA-Z])(?=.*[^a-zA-Z0-9_]).*$)|(^.*(?=.{10,15})(?=.*[0-9])(?=.*[a-zA-Z])|(?=.{10,15})(?=.*[^a-zA-Z0-9_])(?=.*[a-zA-Z])|(?=.{10,15})(?=.*[^a-zA-Z0-9_])(?=.*[0-9]).*$)/" required="required"/>
