		...
		<div class="contents" data-ng-cloak>
	      	...
	      	<table class="type1" style="margin-top:0;">
				...
				<tr>
					<th>검색구분</th>
					<td>
						<select title="회원 검색조건 선택" class="normal w175" data-ng-model="param.condition">
			            	<option value="title">권한명</option>
			          	</select>
					</td>
				</tr>
				<tr>
			...
			<table class="type1">
	          	...
	          	<tr data-ng-repeat="item in board.list">
	            	...
	            	<td class="member_nm pointcolor left">{{item.title}}</td>
	            	<td class="center">{{item.add_cnt}}</td>
	            	<td class="center">{{item.mod_cnt}}</td>
	            	<td class="center">{{item.view_cnt}}</td>
	            	<td class="center">{{item.reg_dt|myDate:'yyyy-MM-dd'}}</td>
	            	<td class="center">
	            		<input type="button" value="수정" class="bt_small_bt4" data-my-href="/modify/{{item.seq}}">
	            	</td>
	            	<td class="center">
	            		<input type="button" value="삭제" class="bt_small_bt7" data-ng-click="del(item)" ng-if="item.seq!=0 && item.seq!=1">
	            	</td>
	          	</tr>
	        </table>
			...
			<div class="btn_bottom">
		    	<div class="r_btn">
					<input type="button" value="권한생성" class="bt_big_bt4" data-my-href="/write"/>
		        </div>
		    </div>
		</div>
