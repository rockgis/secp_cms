...
var selectItem = function(data) {
	...
	if(window.opener || window.tinyMCEPopup || $.urlParam('field_name') || $.urlParam('CKEditorCleanUpFuncNum') || $.urlParam('CKEditor') || $.urlParam('SE2') || $.urlParam('CrossEd3_5')) {
	 ...
	 // tinymce 4 and colorbox
		...
		}else if($.urlParam('SE2')){//스마트 에디터2 추가
			if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
				//스마트 에디터 플러그인을 통해서 넣는 방법 (oFileInfo는 Array)
				opener.nhn.husky.PopUpManager.setCallback(window, 'SET_IMG_PHOTO', [url]);
				//본문에 바로 tag를 넣는 방법 (oFileInfo는 String으로 <img src=....> )
				//opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', [oFileInfo]);
			}
		}else if($.urlParam('CrossEd3_5')){//크로스 에디터3.5 추가
			if (!!opener) {
				opener.CE_OnPaste($.urlParam('CrossEd3_5'), url);
			}
		} else {
