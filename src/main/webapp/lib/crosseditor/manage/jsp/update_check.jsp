...
<%
	...
	if(update_xml.exists()){
		...
		if(update_check_value){
			result_pop = "<script language=\"javascript\">alert(NamoSELang.pe_Ou)</script>";
			String saveCheck = update_xml(update_xml_url,before_xml_url);
			if(saveCheck.equals("sucess")){
				result_sc = resultStrSc;
			}
			else{
				result_sc = "<script language=\"javascript\">alert(NamoSELang.pe_HX+NamoSELang.pe_sK);window.document.location.href='manager_setting.jsp';</script>";
			}
		}
		else{
			result_sc = resultStrSc;
